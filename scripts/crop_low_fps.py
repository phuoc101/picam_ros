...
class ImageCropperLowFPS(object):
    """
    ...
    def __init__(self):
        ...
        self.output_path = os.path.join(
            ...
            "extracted_data",
            rospy.get_param("~output_path", "Tierankatu"),
        )
        self.img_path = os.path.join(self.output_path, "frames")
        self.video_path = os.path.join(self.output_path, "video")
        if not (os.path.isdir(self.img_path) and os.path.islink(self.img_path)):
            os.makedirs(self.img_path, exist_ok=True)
        if not (os.path.isdir(self.video_path) and os.path.islink(self.video_path)):
        ...
        self.yaml_file = os.path.join(self.config_path, rospy.get_param("~yaml", "default.yaml"))
        os.makedirs(self.config_path, exist_ok=True)

        self.frame_cnt = 0
        self.cnt_lim = rospy.get_param("~cnt_lim", 15)
        self.img_id = 0
...
def main():
    ...
    image_cropper = ImageCropperLowFPS()
    image_cropper.get_crop_low_fps()
