...
class ImageUwbSync(object):
    """
    Image Uwb Sync class: Detects contour of fisheye (GUI or load yaml) circle and crop
    image wrt it
    ...
    def __init__(self):
        ...
        self.output_path = os.path.join(
            rospack.get_path("picam_ros"), "data", rospy.get_param("~output_path")
        )
        ...
        self.bagfile = os.path.join(self.bag_path, rospy.get_param("~bagfile"))
        ...
        self.yaml_file = os.path.join(self.config_path, rospy.get_param("~yaml"))
        ...
        for _, msg_img, t_img in self.bag.read_messages(self.image_topic):
            ...
            if self.uwb_topics:
                for i, topic in enumerate(self.uwb_topics):
                    for _, msg_pos, t_pos in self.bag.read_messages(
                        topic, start_time=self.__t_prev[i]
                    ):
                        if t_pos >= t_img:
                            rospy.logdebug(
                                f"Tag: {self.uwb_tags[i]} Closest Coord x: {msg_pos.pose.position.x} y: {msg_pos.pose.position.y}"
                            )
                            anno[
                                i
                            ] = f"{self.uwb_tags[i]} {msg_pos.pose.position.x} {msg_pos.pose.position.y}\n"
